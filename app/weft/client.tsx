...
import { WeavePreview } from "@/components/weave-preview";
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto w-full max-w-4xl flex-1 px-5 pt-6">
        ...
        />
        <p className="mt-6 font-mono text-xs uppercase tracking-[0.14em] text-ink-mute">Onchain index fund · proof basket</p>
        <h1 className="mt-1 font-display text-3xl tracking-tight sm:text-4xl">The Proof Basket</h1>
        <p className="mt-2 max-w-xl text-[15px] leading-relaxed text-ink-soft">
          A live index on Base Vibenet holding two real B20 stock tokens. Weave
          tUSDC in, watch dividends and splits land through the onchain
          multiplier, unwind back out. When the mainnet basket deploys, this is
          the exact interface it runs on.
        </p>

        {/* LIVE BASKET: always on, always action-capable */}
          ...
          <table className="tnum w-full min-w-[560px] text-sm">
            ...
            <tbody className="font-mono text-[13px]">
              <tr className="border-b border-line-soft/60">
                <td className="px-4 py-2.5 text-ink">wAAPL</td>
                <td className="px-4 py-2.5 text-ink-soft">50%</td>
                <td className="px-4 py-2.5 text-right text-ink">
                  x{liveState.ok ? liveState.multA.toFixed(2) : "1.02"}
                </td>
                <td className="px-4 py-2.5 text-right text-ink-soft">$200.00</td>
                <td className="px-4 py-2.5 text-right text-[#2EBD85]">dividend landed</td>
              </tr>
              <tr>
                <td className="px-4 py-2.5 text-ink">wNVDA</td>
                <td className="px-4 py-2.5 text-ink-soft">50%</td>
                <td className="px-4 py-2.5 text-right text-ink">
                  x{liveState.ok ? liveState.multB.toFixed(2) : "2.04"}
                </td>
                <td className="px-4 py-2.5 text-right text-ink-soft">$100.00</td>
                <td className="px-4 py-2.5 text-right text-[#2EBD85]">split landed</td>
              </tr>
            </tbody>
          </table>
        </section>

        {/* LIVE FUND LINE + PREVIEW */}
        <section className="ring-hairline mt-4 flex flex-wrap items-center justify-between gap-3 bg-paper-raise px-4 py-3 text-sm">
          ...
          <span className="font-mono text-xs text-ink-mute">
            {liveState.ok ? `${liveState.symbol} · reads live from Vibenet` : "reading Vibenet..."}
          </span>
        </section>

        {/* WEAVE PREVIEW: works for everyone, wallet or not */}
        <WeavePreview />
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="font-mono text-[11px] uppercase tracking-[0.12em] text-ink-mute">
                Proven · same contract as mainnet target
              </p>
              <h2 className="mt-1 font-display text-xl">The full run</h2>
              <p className="mt-2 max-w-lg text-sm leading-relaxed text-ink-soft">
                $1,000 woven in. Both dividends land. A 2-for-1 split lands.
                Unwound. Six transactions, every line linking to its proof.
              </p>
            </div>
            <Link
              href="/weft/proof"
        ...
        {/* mainnet rail: cold standby until deployment */}
        <section className="ring-hairline mt-4 bg-paper-raise p-6">
          ...
          <p className="mt-3 text-[13px] leading-relaxed text-ink-mute">
            The same WeftIndex bytecode that ran the proof run above will
            deploy against these pools on mainnet. This card is reference only
            until then.
          </p>
        </section>
